...
Cart Service is deployed as FaaS. The code for Azure functions is available in the directory azure/cart_svc_serverless.

Also, spin up a REDIS store before using the cart service. In this example, I am using Azure Cache for REDIS. 

Make sure to add REDIS_HOST, REDIS_PORT and REDIS_PASSWORD environment variable to azure functions. 
...
Before deploying the front end, edit the frontend-total.yaml and add the environment variables associated with the CART service. These can be obtained from the Azure Portal.
...
If you want the entire manifest file to deploy the application solely on docker-compose, aws-fargate or kubernetes environment, refer to [this](https://github.com/ishrivatsa/acme_fitness_demo) repo for info. 
